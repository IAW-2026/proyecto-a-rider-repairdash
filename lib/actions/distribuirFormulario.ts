...
export async function distribuirFormulario(formData: FormData): Promise<void> {
  ...
  if (result) {
    ...
    console.log("[distribuirFormulario] promocionId del form:", promocionId, "userId:", cliente.id_clerk);
    if ( promocionId) {
      ...
      for (const foto of fotos) {
        ...
        if (error) {
          console.error("[distribuirFormulario] error subiendo foto:", error.message);
          continue;
        }

        const { data } = supabaseServer.storage.from("trabajos").getPublicUrl(path);
      ...
      try {
        ...
        console.error("[realizarFetchDriver] fallo, haciendo rollback del viaje:", err);
        await cancelarViaje(result.id_viaje);
        throw new Error("No se pudo contactar al servicio de drivers. Por favor intentá de nuevo.");
      }
