import { countViajesByClerkID } from "@/lib/queries/viajes";
...
export async function GET(
  ...
  const total = await countViajesByClerkID(idClerk);
  return new Response(JSON.stringify({ data: { idClerk, total } }), {
    status: 200,
    headers: { "Content-Type": "application/json" },
  });
}
